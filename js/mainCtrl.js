angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    //Code here
    $scope.getMusic = function(artist) {
      itunesService.getMusic(artist).then(function(response) {
        var finalArray = [];
        for(var i = 0; i < response.length; i++) {
          finalArray.push({
            ...
            MusicVideo: response[i].trackViewUrl,
  ...
  //is sort the data you got back to be an object in the following format.
    /*
      ...
      Type: "song"
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and then loop through the
  //iTunes data pushing into your new array objects that look like the above data.
